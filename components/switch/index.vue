<template>
  <div class="vcu-switch-wrap vcu-cell vcu-cell_switch">
    <div class="vcu-cell__bd">
      <label class="vcu-label" :style="labelStyle" :class="labelClass" v-html="title"></label>
      <inline-desc v-if="inlineDesc">{{ inlineDesc }}</inline-desc>
    </div>
    <div class="vcu-cell__ft">
      <input class="vcu-switch" type="checkbox" :disabled="disabled" v-model="currentValue" />
      <div v-if="preventDefault" class="vcu-switch-overlay" @click="onClick"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    labelClass() {
      return {
        'vcu-cell-justify': this.$parent && this.$parent.labelAlign === 'justify'
      }
    }
  },
  methods: {
