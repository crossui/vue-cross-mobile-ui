<template>
  <cell @click.native="show" :title="title" :value="currentValue" :is-link="!readonly" :value-align="valueAlign" :border-intent="borderIntent">
    <span class="vcu-cell-placeholder" v-if="!displayValue && placeholder">{{ placeholder }}</span>
    <span class="vcu-cell-value" v-if="displayValue">{{ displayValue }}</span>
    <span slot="icon">
      <slot name="icon"></slot>
    </span>
    <div v-transfer-dom>
        ...
        <radio :options="options" v-model="currentValue" :fill-mode="false" @on-change="onValueChange">
          <template slot="each-item" slot-scope="props">
            <slot name="each-item" :icon="props.icon" :label="props.label" :index="props.index">
              <p>
                <img class="vcu-radio-icon" :src="props.icon" v-show="props.icon">
                <span class="vcu-radio-label">{{ props.label }}</span>
              </p>
            </slot>
          </template>
        </radio>
