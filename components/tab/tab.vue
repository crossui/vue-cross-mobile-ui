<template>
  <div class="vcu-tab-wrap" :class="barPosition === 'top' ? 'vcu-tab-bar-top' : ''">
    <div class="vcu-tab-container">
      <div class="vcu-tab" :class="[{'vcu-tab-no-animate': !animate},{ scrollable }]" ref="nav">
        <slot></slot>
        <div v-if="animate" class="vcu-tab-ink-bar" :class="barClass" :style="barStyle">
          <span class="vcu-tab-bar-inner" :style="innerBarStyle" v-if="customBarWidth"></span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    barClass() {
      return {
        'vcu-tab-ink-bar-transition-forward': this.direction === 'forward',
        'vcu-tab-ink-bar-transition-backward': this.direction === 'backward'
      }
    },
    scrollable() {
