      ...
      if (value.length < count) {
        for (let i = 0; i < count; i++) {
          if (process.env.NODE_ENV === 'development' &&
            ...
            !this.columns) {
            console.error('[vcu error] 渲染出错，如果为联动模式，需要指定 columns(列数)')
          }
