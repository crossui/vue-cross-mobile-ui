...
if (isBrowser) {
    window.__$vcuPopups = window.__$vcuPopups || {}
}
...
const popupDialog = function(option) {
    ...
    if (!!document.querySelectorAll('.vcu-popup-mask').length <= 0) {
        this.divMask = document.createElement('a')
        this.divMask.className = 'vcu-popup-mask'
    ...
    div.className += ` vcu-popup-dialog vcu-popup-dialog-${this.uuid}`
    if (!this.params.hideOnBlur) {
        div.className += ' vcu-popup-mask-disabled'
    }
    ...
    this.container = document.querySelector('.vcu-popup-dialog-' + this.uuid)
    this.mask = document.querySelector('.vcu-popup-mask')
...
popupDialog.prototype.show = function() {
    if (this.params.showMask) {
        this.mask.classList.add('vcu-popup-show')
        this.mask.style['zIndex'] = 500
    }
    this.container.classList.add('vcu-popup-show')
    this.params.onOpen && this.params.onOpen(this)
    if (isBrowser) {
        window.__$vcuPopups[this.uuid] = 1
    }
}

popupDialog.prototype.hide = function(shouldCallback = true) {
    this.container.classList.remove('vcu-popup-show')
    if (!document.querySelector('.vcu-popup-dialog.vcu-popup-show')) {
        this.mask.classList.remove('vcu-popup-show')
        setTimeout(() => {
    ...
    this.isShow = false
    if (isBrowser) {
        delete window.__$vcuPopups[this.uuid]
    }
}
