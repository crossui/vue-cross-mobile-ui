    ...
    <div class="calendar-header" v-show="!hideHeader">
      <div class="calendar-year">
        <span @click="go(year - 1, month)">
          <a class="year-prev vcu-prev-icon" href="javascript:"></a>
        </span>
        ...
        <span class="calendar-header-right-arrow" @click="go(year + 1, month)">
          <a class="year-next vcu-next-icon" href="javascript:"></a>
        </span>
      </div>

      <div class="calendar-month">
        <span @click="prev">
          <a class="month-prev vcu-prev-icon" href="javascript:"></a>
        </span>
        ...
        <span @click="next" class="calendar-header-right-arrow">
          <a class="month-next vcu-next-icon" href="javascript:"></a>
        </span>
      </div>
    </div>
      ...
      <tbody>
        <tr v-for="(day,k1) in days">
          <td v-for="(child,k2) in day" :data-date="formatDate(year, month, child)" :data-current="currentValue" :class="buildClass(k2, child)" @click="select(k1, k2, child)">
            <slot :year="year" :month="month" :child="processDateItem(child)/* deprecated, use date instead */" :date="processDateItem(child)" class-name="vcu-calendar-each-date" :row="k1" :col="k2" :raw-date="formatDate(year, month, child)" :show-date="replaceText(child.day, formatDate(year, month, child))" :is-show="showChild(year, month, child)" name="each-day">
              <span class="vcu-calendar-each-date" :style="getMarkStyle(child)" v-show="showChild(year, month, child)">
                {{ replaceText(child.day, formatDate(year, month, child)) }}
                <span class="vcu-calendar-top-tip" v-if="isShowTopTip(child)" :style="isShowTopTip(child, 'style')">
                  <span>{{ isShowTopTip(child, 'text') }}</span>
                </span>
              </span>
              <span class="vcu-calendar-dot" v-if="isShowBottomDot(child)"></span>
...
export default {
  ...
  computed: {
    _weeksList() {
      ...
      if (!this.weeksList || !this.weeksList.length) {
        // tip for older vcu-loader
        if (typeof V_LOCALE === 'undefined') {
          /* if (process.env.NODE_ENV === 'development') {
            console.warn('[vcu warn] 抱歉，inline-calendar 组件需要升级 vcu-loader 到最新版本才能正常使用')
          } */
